                ...
                container('node') {
                    script {
                        ...
                        withCredentials([string(credentialsId: 'tpausl-github-token', variable: 'GITHUB_TOKEN')]) {
                            ...
                            sh "curl -XPOST -H \"Authorization:token ${GITHUB_TOKEN}\" -H \"Content-Type:application/octet-stream\" --data-binary @dist_keycloak/keycloak-theme-for-kc-all-other-versions https://uploads.github.com/repos/${repo}/releases/${releaseId}/assets?name=keycloak-theme-for-kc-all-other-versions.jar"
                        }
                    }
                }
            }
