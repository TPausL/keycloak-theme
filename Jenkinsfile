    ...
    stages {
        ...
        stage('Create release') {
            steps {
                container('node') {
                    script {
                        createGitHubRelease(credentialId: 'tpausl-github-token', repo: 'tpausl/keycloak-theme', tag: env.GIT_COMMIT.take(7), commitish: env.GIT_COMMIT)
                        uploadGitHubReleaseAsset(credentialId: 'tpausl-github-token', repo: 'tpausl/keycloak-theme', uploadAssets: [
